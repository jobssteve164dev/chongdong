  ...
  applyNetworkSettingsToConfig(config, networkSettings) {
    ...
    if (networkSettings.enableDns && finalConfig.dns) {
      ...
      if (networkSettings.enableDnsCache) {
        finalConfig.dns.cache_size = networkSettings.dnsCacheSize || 1e3;
  ...
  setupProcessHandlers() {
    ...
    electron.app.on("gpu-process-crashed", (_event, killed) => {
      this.handleProcessCrash({
        ...
        context: {
          killed,
          gpuInfo: electron.app.getGPUInfo("basic")
        }
      });
    });
...
electron.ipcMain.handle("geolocation:testViaProxy", async (_, { proxyUrl }) => {
  try {
    ...
    const proxyPort = Number(appSettings == null ? void 0 : appSettings.proxyPort) || Number(proxyUrlObj.port) || 7890;
    const socksPort = Number(appSettings == null ? void 0 : appSettings.socksPort) || 7891;
