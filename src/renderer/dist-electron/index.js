...
class VpnServerManager {
   ...
   * 启动OpenVPN服务器
   */
  async startOpenVpnServer(config) {
    const configPath = path$1.join(this.configDir, "openvpn-server.conf");
    const openVpnConfig = this.generateOpenVpnConfig(config);
    fs$1.writeFileSync(configPath, openVpnConfig);
    this.vpnProcess = child_process.spawn("openvpn", [
      "--config",
      configPath,
      "--daemon"
    ]);
   ...
   * 启动WireGuard服务器
   */
  async startWireGuardServer(config) {
    const configPath = path$1.join(this.configDir, "wg0.conf");
    const wireGuardConfig = this.generateWireGuardConfig(config);
    fs$1.writeFileSync(configPath, wireGuardConfig);
    this.vpnProcess = child_process.spawn("wg-quick", ["up", configPath]);
    this.vpnProcess.on("error", (error) => {
  ...
  async configureSystemVpn() {
    ...
    try {
      const vpnConfig = {
        ...
        type: this.currentConfig.type === "openvpn" ? "l2tp" : "openvpn",
        // 根据虫洞VPN服务器类型选择系统VPN类型
        username: "chongdong",
